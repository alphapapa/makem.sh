...
+  Eldev expects (though it does not strictly require) some initialization and configuration for each project before use.  =makem.sh= is designed to work without initialization or configuration.
+  Eldev appears to use the developer's local Emacs configuration for installing package dependencies (it is difficult to discern exactly how this works from the documentation).  =makem.sh= has built-in, optional sandboxing to install dependencies separately from the developer's Emacs configuration.
...
+  =emake= provides some tools for building Emacs versions locally and on CI systems.  =makem.sh= itself uses only the locally installed version of Emacs; a GitHub Actions configuration is provided that uses other tools to install Emacs versions on CI systems.
